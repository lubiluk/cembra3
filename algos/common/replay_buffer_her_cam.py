...
class ReplayBuffer:
    ...
    def __init__(
        ...
        self.obs_camera_buf = torch.zeros(
            combined_shape(size, camera_dim), dtype=torch.float32, device=device
        )
        self.obs_robot_state_buf = torch.zeros(
            combined_shape(size, robot_state_dim), dtype=torch.float32, device=device
        )
        self.obs_dgoal_buf = torch.zeros(
            combined_shape(size, goal_dim), dtype=torch.float32, device=device
        )
        self.obs_agoal_buf = torch.zeros(
            combined_shape(size, goal_dim), dtype=torch.float32, device=device
        )
        ...
        self.obs2_camera_buf = torch.zeros(
            combined_shape(size, camera_dim), dtype=torch.float32, device=device
        )
        self.obs2_robot_state_buf = torch.zeros(
            combined_shape(size, robot_state_dim), dtype=torch.float32, device=device
        )
        self.obs2_dgoal_buf = torch.zeros(
            combined_shape(size, goal_dim), dtype=torch.float32, device=device
        )
        self.obs2_agoal_buf = torch.zeros(
            combined_shape(size, goal_dim), dtype=torch.float32, device=device
        )
    ...
    def store(self, obs, act, rew, next_obs, done, info):
        ...
        obs_robot_state = obs["observation"]["robot_state"]
        obs_dgoal = obs["desired_goal"]
        obs_agoal = obs["achieved_goal"]

        next_obs_camera = next_obs["observation"]["camera"]
        next_obs_robot_state = next_obs["observation"]["robot_state"]
        next_obs_dgoal = next_obs["desired_goal"]
        next_obs_agoal = next_obs["achieved_goal"]

        self.obs_camera_buf[self.ptr] = torch.as_tensor(obs_camera, dtype=torch.float32)
        self.obs_robot_state_buf[self.ptr] = torch.as_tensor(
            obs_robot_state, dtype=torch.float32
        )
        ...
        self.obs2_camera_buf[self.ptr] = torch.as_tensor(
            next_obs_camera, dtype=torch.float32
        )
        self.obs2_robot_state_buf[self.ptr] = torch.as_tensor(
            next_obs_robot_state, dtype=torch.float32
        )
        self.obs2_dgoal_buf[self.ptr] = torch.as_tensor(
            next_obs_dgoal, dtype=torch.float32
        )
        self.obs2_agoal_buf[self.ptr] = torch.as_tensor(
            next_obs_agoal, dtype=torch.float32
        )
    ...
    def _get_batch(self, idxs):
        obs = {
            "observation": {
                ...
                "robot_state": self.obs_robot_state_buf[idxs],
            },
        ...
        obs2 = {
            "observation": {
                ...
                "robot_state": self.obs2_robot_state_buf[idxs],
            },
