...
class SquashedGaussianMLPActor(nn.Module):
    def __init__(
        ...
        cam_space = obs_space.spaces["observation"]["camera"]
        robot_dim = obs_space.spaces["observation"]["robot_state"].shape[0]
        ...
        self.net = mlp(
            [feature_dim + robot_dim + dgoal_dim] + list(hidden_sizes),
            activation,
            activation,
        )
    ...
    def forward(self, obs, deterministic=False, with_logprob=True):
        ...
        net_out = self.net(torch.cat((feat, obs_robot_state, dgoal), dim=-1))
...
class MLPQFunction(nn.Module):
    def __init__(
        ...
        self.q = mlp(
            [feature_dim + robot_dim + dgoal_dim + act_dim] + list(hidden_sizes) + [1],
            activation,
        )
    ...
    def forward(self, obs, act):
        ...
        q = self.q(torch.cat([feat, obs_robot_state, dgoal, act], dim=-1))
...
class MLPActorCritic(nn.Module):
    ...
    def act(self, obs, deterministic=False):
        with torch.no_grad():
            obs["observation"] = {
                "camera": torch.as_tensor(obs["observation"]["camera"], dtype=torch.float32).unsqueeze(
                    dim=0
                ),
                "robot_state": torch.as_tensor(
                    obs["observation"]["robot_state"], dtype=torch.float32
                ).unsqueeze(dim=0),
            }
            obs["desired_goal"] = torch.as_tensor(
                obs["desired_goal"], dtype=torch.float32
            ).unsqueeze(dim=0)
            obs["achieved_goal"] = torch.as_tensor(
                obs["achieved_goal"], dtype=torch.float32
            ).unsqueeze(dim=0)
